...
export function TripProvider({ children }: { children: ReactNode }) {
  const [tripParams, setTripParams] = useState<TripParams | null>(() => readStored(STORAGE_KEYS.tripParams))
  const [resolvedItinerary, setResolvedItinerary] = useState<ResolvedItinerary | null>(() => readStored(STORAGE_KEYS.resolvedItinerary))
  const [selectedVariant, setSelectedVariant] = useState<string | null>(() => readStored(STORAGE_KEYS.selectedVariant))
  const [status, setStatus] = useState<TripStatus>('idle') // idle | loading | success | error
  ...
  const [pendingRecomputes, setPendingRecomputes] = useState<Set<string>>(() => new Set())
  ...
  const recomputeRequestIds = useRef<Record<string, number>>({})

  function dayRecomputeKey(variantKey: string, dayIndex: number) {
  ...
  // one.
  const recomputeDayTravelTimes = useCallback((variantKey: string, dayIndex: number, items: ItineraryItem[], destination: string | undefined, transport: string | undefined) => {
  ...
  const reorderDayItem = useCallback((variantKey: string, dayIndex: number, itemIndex: number, direction: number) => {
  ...
  // back.
  const swapDayItem = useCallback((variantKey: string, dayIndex: number, itemIndex: number, replacement: Partial<ItineraryItem>) => {
    let updatedItems: ItineraryItem[] | null = null
    setResolvedItinerary((prev) => {
      if (!prev || !prev[variantKey]) return prev
      const variant = prev[variantKey]
  ...
  const loadSavedItinerary = useCallback((data: ResolvedItinerary) => {
    setErrorMessage(null)
    setResolvedItinerary(data)
    setStatus('success')
  }, [])
