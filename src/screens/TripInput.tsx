...
export default function TripInput() {
  ...
  const isRestoringInterests = !cameFromTrendingCard && (tripParams?.interestOptions?.length ?? 0) > 0
  const [interestOptions, setInterestOptions] = useState<string[]>(
    isRestoringInterests ? tripParams?.interestOptions ?? STAPLE_INTERESTS : STAPLE_INTERESTS
  )
  ...
  // destination.
  const generatedForRef = useRef<string | null>(isRestoringInterests ? tripParams?.destination ?? null : null)
  ...
  useEffect(() => {
    ...
    clearTimeout(suggestionsDebounceRef.current ?? undefined)
    ...
    return () => clearTimeout(suggestionsDebounceRef.current ?? undefined)
  }, [destination])
