...
// Shapes of the Google Places Autocomplete (New) response the /api/place-
// autocomplete proxy returns, narrowed to just the fields this component reads.
...
function HighlightedText({ text, matches }: { text: string; matches?: HighlightMatch[] }) {
  if (!matches || matches.length === 0) {
    return <span className="autocomplete-item__rest">{text}</span>
  }

  const sorted = [...matches].sort((a, b) => (a.startOffset ?? 0) - (b.startOffset ?? 0))
