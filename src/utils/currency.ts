...
function unitFor(amount: number): CompactUnit | null {
...
export function formatCompactAmount(
  ...
  { unit, fractionDigits = 1, currencyCode }: { unit?: CompactUnit | null; fractionDigits?: number; currencyCode?: string | null } = {}
) {
  if (amount == null) return ''
...
export function formatCompactRange(range: PriceRange | null | undefined, { currencyCode }: { currencyCode?: string | null } = {}) {
...
export function formatExactRange(range: PriceRange | null | undefined) {
  if (!range) return ''
  const exact = new Intl.NumberFormat('en', { maximumFractionDigits: 0 })
  const amount =
    range.min === range.max
