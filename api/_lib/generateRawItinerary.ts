...
- NO REPEATS (strictly enforced): never more than one stop for the same interest on the same day, and across the whole trip at most ${Math.max(1, Math.ceil(days / 2))} stop${Math.max(1, Math.ceil(days / 2)) === 1 ? '' : 's'} for any one interest. Two listings for one venue count as one stop and may not both appear: not the square and the building on it, not the harbour and the marina in it, not a casino and the casino's cafe. Choose the best one and move on to another interest.
- HOTELS ARE NOT STOPS. The only hotel in the plan is the accommodation. Never list a hotel, resort or beach club that is part of a hotel as an activity, however famous it is, and never send the traveller to look at a hotel. If the thing worth seeing is inside a hotel (a casino, a bar, a restaurant, a spa), name that thing, not the hotel.
