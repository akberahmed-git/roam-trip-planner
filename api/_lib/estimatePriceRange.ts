...
export async function estimatePriceRanges({ destination, currencyCode, hotelsByTier }: { destination: string; currencyCode: string; hotelsByTier: Record<string, Array<{ name?: string }>> }) {
...
Only include tiers that were listed above. Every tier listed must get a real, specific numeric estimate - never omit a tier or return null.`;
  ...
  // Anthropic content blocks are a union; only a text block carries `.text`,
  // so narrow before reading it rather than assuming the first block is text.
  ...
  try {
    ...
    const err = new Error('Claude did not return valid JSON for a price estimate') as Error & { rawText?: string };
    err.rawText = rawText;
    throw err;
  }
